...
description: គ្រប់គ្រងអ្វីដែល program ធ្វើ ដោយផ្អែកលើ conditions ដោយប្រើ if, elif, និង else, រួមទាំង conditional expression (ternary operator)។
objectives:
...
order: 1
quiz:
  - question: What is the correct syntax for the ternary (conditional) expression in Python?
    options:
      - "x if condition ? y"
      - "condition ? x : y"
    ...
    correctAnswer: 2
    explanation: Python's conditional expression is written as `value_if_true if condition else value_if_false`. There is no ternary `?:` operator in Python.
  - question: How many elif branches can a single if statement have?
    options:
      ...
      - "Unlimited"
    correctAnswer: 3
    explanation: Python places no limit on the number of elif branches in a single if/elif/else chain.
---

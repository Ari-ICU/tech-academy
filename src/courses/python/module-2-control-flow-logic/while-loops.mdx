...
description: ប្រើ while loops សម្រាប់ repetition ដោយផ្អែកលើ condition ហើយចាប់ផ្តើមជាម្ចាស់ជាមួយ break, continue, និង else clause។
objectives:
...
order: 3
quiz:
  - question: What happens if a while loop's condition never becomes False and there is no break?
    options:
    ...
    correctAnswer: 1
    explanation: Python has no built-in loop time limit. If the condition is always True and there is no break, the loop runs until the process is killed.
  - question: When does the else clause of a while loop execute?
    ...
    correctAnswer: 1
    explanation: The else clause runs only when the condition becomes False naturally — it is skipped if the loop exits via break.
---

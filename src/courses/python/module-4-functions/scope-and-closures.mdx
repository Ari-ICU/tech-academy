...
```text
Built-in (របស់មានស្រាប់ក្នុង Python ដូចជា print, len)
     ▲
     │
Global (អថេរនៅខាងក្រៅបំផុត ឧ. name = "Python")
     ▲
     │
Enclosing (នៅក្នុង Function ខាងក្រៅ def outer():)
     ▲
     │
Local (នៅក្នុង Function បច្ចុប្បន្ន def inner():)
```
...
├── បង្កើត count = 0
│
└── បញ្ជូនលទ្ធផលត្រឡប់ជាអនុគមន៍ increment()

        │
        ▼

Function ខាងក្រៅចប់ ប៉ុន្តែវានៅតែរក្សាទុកសេចក្តីយោង (keeps reference)
...
```text
Function ដើមរបស់អ្នក (Your Function)
      │
      ▼
ឆ្លងកាត់ Decorator
      │
      ▼
ធ្វើអ្វីមួយមុន (Before)
      ↓
រត់កូដ Function ដើម (Original Function)
      ↓
ធ្វើអ្វីមួយក្រោយ (After)
      │
      ▼
លទ្ធផលចុងក្រោយ (Result)
```

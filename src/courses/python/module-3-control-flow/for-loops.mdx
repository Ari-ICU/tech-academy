...
description: Iterate លើ sequences, ranges, និង iterable ណាមួយជាមួយ for loop, ហើយប្រើ enumerate(), zip(), និង list comprehensions។
objectives:
...
quiz:
  - question: What does list(range(2, 10, 3)) produce?
    options:
      ...
      - "[2, 4, 6, 8]"
    correctAnswer: 1
    explanation: range(2, 10, 3) starts at 2 and steps by 3 — giving 2, 5, 8. The next value (11) exceeds stop (10) so it is excluded.
  - question: What does enumerate(['a', 'b', 'c']) yield on each iteration?
    ...
    correctAnswer: 1
    explanation: enumerate() yields (index, value) tuples, defaulting to a start index of 0.
---

...
description: រៀនប្រើ Dictionary (Key-Value Pairs) និង Set (Unique Values) ដើម្បីរក្សាទុក និងស្វែងរកទិន្នន័យបានលឿន។
objectives:
...
quiz:
  - question: What does dict.get(key, default) return when the key does not exist?
    options:
      ...
      - Returns False
    correctAnswer: 2
    explanation: dict.get() returns the default value (or None if omitted) instead of raising KeyError.

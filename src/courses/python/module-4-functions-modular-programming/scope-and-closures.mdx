...
description: >-
  យល់ពីរបៀបដែល Python resolve names ជាមួយ LEGB rule, របៀប closures capture
  state, និងរបៀប decorators wrap functions។
objectives:
...
quiz:
  - question: >-
      What keyword allows a nested function to modify a variable in its
    ...
    correctAnswer: 1
    explanation: >-
      nonlocal allows a nested function to rebind a variable in the nearest
      enclosing (non-global) scope.
  - question: What is a closure?
    options:
      - A function that has no parameters
      - >-
        A function that remembers the variables from its enclosing scope even
        after that scope has returned
      - A class with a __call__ method
      - A function that cannot be called more than once
    correctAnswer: 1
    explanation: >-
      A closure is a nested function that retains references to variables in its

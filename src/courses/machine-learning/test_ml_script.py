...
def process_file(filepath):
    ...
    # In Depth block
    in_depth_str = """When studying \*\*(.*?)\*\*, focus on the following details:
...
3. \*\*Best Practices\*\*: Avoiding common pitfalls such as leakage, target drift, or overfitting\."""
    ...
    in_depth_rep = """នៅពេលសិក្សាអំពី **\\1** សូមផ្តោតលើចំណុចលម្អិតខាងក្រោម៖
...
3. **ការអនុវត្តល្អបំផុត (Best Practices)**៖ ជៀសវាងកំហុសទូទៅដូចជា ការលេចធ្លាយទិន្នន័យ (Data leakage), target drift, ឬ overfitting។"""
    ...
    # Fallback to remove any khmerblock tags that were missed (though they should have been replaced)
    content = re.sub(r'<khmerblock>\n(.*?)\n</khmerblock>', r'\1', content, flags=re.DOTALL)

    print(content)

process_file('/Users/thoeurnratha/Documents/web-development/data-science/platform/src/courses/machine-learning/ml-module-1-introduction/applications-of-ml.mdx')
